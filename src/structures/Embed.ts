...
export class Embed<Res extends APIMessage | null = APIMessage> {
  ...
  constructor(private readonly sendback?: (embed: Embed<Res>, reply: boolean, mention: boolean, ephemeral: boolean) => Promise<Res>) { }
   ...
   */
  async send(reply: boolean = true, mention = false, ephemeral = false): Promise<Res> {
    if (!this.sendback) throw new Error('No sendback function, so could not run Embed.send()')
    return await this.sendback(this, reply, mention, ephemeral)
  }
