...
export class CommandContext {
   ...
   * @param ephemeral Whether to send the message so only the user can see (only available in slash commands)
   * @returns Message sent
   */
  async reply(data: MessageTypes, mention = false, ephemeral: boolean = false): Promise<APIMessage> {
    if (!mention) {
